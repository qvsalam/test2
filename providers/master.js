// ============================================================
// Iraq Scrapers — master.js v4.5.0
// VODU + CinemaBox + Cinemana
// ✅ يشتغل الثلاثة بالتوازي — fallback تلقائي
// ✅ عناوين عربية + إنجليزية من TMDB
// ✅ مطابقة بالسنة لتجنب الخلط
// ✅ بدون require() — متوافق مع Nuvio sandbox
// ============================================================
...
var CINEMANA = "https://cinemana.shabakaty.com/api/android/";
...
function cinemanaSearch(titles, year, idx, type, season, episode) {
  ...
  return fetch(CINEMANA + "AdvancedSearch?videoTitle=" + encodeURIComponent(titles[idx]) + "&type=" + type)
    .then(function(r) { return r.json(); })
    .then(function(results) {
      if (!results || !results.length)
        return cinemanaSearch(titles, year, idx + 1, type, season, episode);
      var match = pickBestMatch(results, titles, year);
      if (!match) return cinemanaSearch(titles, year, idx + 1, type, season, episode);
      if (type === "series" && season && episode)
        return cinemanaTVFiles(match.nb, parseInt(season) || 1, parseInt(episode) || 1);
      return cinemanaFiles(match.nb);
    })
    .catch(function() { return cinemanaSearch(titles, year, idx + 1, type, season, episode); });
}

function cinemanaTVFiles(showNb, sNum, eNum) {
  return fetch(CINEMANA + "videoSeason/id/" + showNb)
    .then(function(r) { return r.json(); })
...
function cinemanaFiles(nb) {
  return fetch(CINEMANA + "transcoddedFiles/id/" + nb)
    .then(function(r) { return r.json(); })
    .then(function(files) {
      var streams = [], seen = {};
...
var CB = "https://cinema.albox.co/api/v4/";
...
function cinemaboxSearch(titles, idx, mediaType, season, episode) {
  if (idx >= titles.length) return Promise.resolve([]);
  return fetch(CB + "search?q=" + encodeURIComponent(titles[idx]))
    .then(function(r) { return r.json(); })
    .then(function(data) {
      ...
      return fetch(CB + "shows/shows/dynamic/" + match.id)
        .then(function(r) { return r.json(); })
...
function cinemaboxPlayer(episodeId) {
  return fetch(CB + "shows/episodes/player/" + episodeId)
    .then(function(r) { return r.json(); })
    .then(function(data) {
      var streams = [], seen = {};
...
function voduTryLinks(links, idx, mediaType, season, episode) {
  ...
  return fetch(links[idx])
    .then(function(r) { return r.text(); })
    .then(function(html) {
      var allUrls = voduGetUrls(html), streams = [], seen = {};
      if ((mediaType === "tv" || mediaType === "series") && season && episode) {
        ...
        for (var i2 = 0; i2 < allUrls.length; i2++) {
          var url2 = allUrls[i2];
          if (voduSkip(url2) || seen[url2]) continue;
          seen[url2] = true;
