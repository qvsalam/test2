...
function TIT(x){return String((x&&(x.name||x.title||x.en_name||x.ar_name||x.original_name||x.post_title||x.show_name||x.show_title))||"")}
...
function EP(x){return NUM(x,["episode_number","episodeNumber","description","title","episode"])}
function SN(x){return NUM(x,["season_number","seasonNumber","season","number","title"])}
function OK(x,ts){var a=N(TIT(x));for(var i=0;i<ts.length;i++){var b=N(ts[i]);if(b&&(a==b||a.indexOf(b)>=0||b.indexOf(a)>=0))return true}return false}
function V(data){var out=[];(function walk(x){if(!x)return;if(Array.isArray(x)){for(var i=0;i<x.length;i++)walk(x[i]);return}if(typeof x==="object"){if(x.url&&/^https?:/.test(x.url)){var q=x.quality||(/1080/.test(x.url)?"1080p":/720/.test(x.url)?"720p":/480/.test(x.url)?"480p":"HD");out.push({title:"CinemaBox "+q,name:"CinemaBox",provider:"CinemaBox",url:x.url,quality:q,type:x.url.indexOf(".m3u8")>-1?"hls":"direct"})}if(x.videos)walk(x.videos)}})(data);return out}
async function TM(id){var u="https://api.themoviedb.org/3/tv/"+id+"?api_key="+K,a=[],e=await J(u+"&language=en").catch(function(){return{}})||{},r=await J(u+"&language=ar").catch(function(){return{}})||{};function add(v){v=String(v||"").trim();if(v&&a.indexOf(v)<0)a.push(v);v=v.replace(/[’'`]/g,"");if(v&&a.indexOf(v)<0)a.push(v)}add(e.name);add(e.original_name);add(r.name);add(r.original_name);return a}
...
async function getStreams(tmdbId,mediaType,season,episode){try{var ts=await TM(tmdbId),s=parseInt(season,10)||1,e=parseInt(episode,10)||1;for(var t=0;t<ts.length;t++){var rs=await S(ts[t]);for(var i=0;i<rs.length;i++)if(ID(rs[i])&&OK(rs[i],ts)){var o=await find(ID(rs[i]),s,e);if(o.length)return o}}return[]}catch(err){return[]}}
